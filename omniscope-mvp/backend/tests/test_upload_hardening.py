...
def test_upload_oversize(tmp_path):
    app = create_app({'TESTING': True, 'UPLOAD_DIR': str(tmp_path), 'UPLOAD_MAX_BYTES': 10})
...
def test_upload_disallowed_type(tmp_path):
    app = create_app({'TESTING': True, 'UPLOAD_DIR': str(tmp_path), 'UPLOAD_ALLOWED_MIMETYPES': ['text/plain'], 'UPLOAD_ALLOWED_EXTENSIONS': ['.txt']})
    client = app.test_client()
