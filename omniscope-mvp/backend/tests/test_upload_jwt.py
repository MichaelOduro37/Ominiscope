...
def test_upload_with_valid_jwt(tmp_path):
    ...
    app = create_app({'TESTING': False, 'UPLOAD_DIR': str(tmp_path), 'RBAC_ENABLED': True, 'UPLOAD_JWT_SECRET': secret})
    client = app.test_client()
    data = {'file': (io.BytesIO(b'hello'), 'ok.txt')}
    rv = client.post('/ingest/upload', data=data, content_type='multipart/form-data', headers={'Authorization': f'Bearer {token}'})
